...
const UpdateDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    // const token = 'b1c1d548d4b71b4495b295fa7bf5cb6e'

    // formData.multipleSelect = formData.multipleSelect.join(',')

    try {
      const token = localStorage.getItem('token') // Adjust key based on where you store the token
      const response = await axios.post('/AgreementDashboard/updateLead', formData, {
        headers: {
          token: token,
        },
      })
  ...
  useEffect(() => {
    ...
    axios
      .post('/AgreementDashboard/getLead', { id }, { headers: { token } })
      .then((response) => {
  ...
  const options = [
    ...
    { label: 'dummy3', value: 'dummy3' },
    { label: 'dummy4', value: 'dummy4' },
  ]
            ...
            <CCard className="mx-4">
              <CCardBody className="p-4">
                <CForm onSubmit={handleSubmit}>
                  <h4 className="text-center">Update Agreement Form</h4>
